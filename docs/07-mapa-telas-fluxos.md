...
| TL-010 | Importar vacinados históricos | Migrar vacinados de anos anteriores (RN-027) — auto-cria campanha "Histórico — {vacina} {ano}" e vacina no catálogo; inline (≤2000) ou fila/worker (>2000) com status | super_admin, operador_interno | **✅** (console admin §10b · `POST /interno/clientes/{id}/vacinados-historico/importar` · `GET /interno/importacoes-historico/{id}`) |
