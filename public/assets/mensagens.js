/* ============================================================================
   public/assets/mensagens.js
   Catálogo ÚNICO das mensagens de erro das importações (lado das telas).

   Antes cada tela montava a própria frase e o mesmo problema chegava ao usuário
   escrito de jeitos diferentes. Aqui o código é a chave estável (é o que a API
   devolve e o que o relatório de erros grava) e a mensagem é o texto exibido.
   ...
   Espelho do backend em app/helpers/mensagens_importacao.php.
...
(function (global) {
  ...
  var TEXTOS = {
    // --- Arquivo e cabeçalho (conferidos antes de enviar) ---
   ...
   * Código desconhecido volta como está — melhor mostrar a sigla do que uma
   * frase inventada que esconde um caso não catalogado.
   */
  function texto(codigo, ctx) {
    var t = TEXTOS[codigo] || codigo;
    Object.keys(ctx || {}).forEach(function (k) {
      t = t.split('{' + k + '}').join(String(ctx[k]));
    });
    return t;
  }

  global.MSG = { texto: texto, TEXTOS: TEXTOS };
